...
def create_point(canvas,x, y, color="black",):
    # Create a small oval to represent the point
    radius = 3  # Point size
    ...
    if len(y) <= 1:
        ...
        for i in range(1, len(x)):
            canvas.create_line(margin + (x[i-1]), margin+height - (y[i-1]),
                            ...
                            fill="#824be3", width=2)
            create_point(canvas, margin + (x[i-1]), margin+height - (y[i-1]), color="#824be3")
            create_point(canvas, margin + x[i], margin+height-y[i], color="#824be3")
